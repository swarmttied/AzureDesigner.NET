...
namespace AzureDesigner.AIContexts.Language
{
    public class LanguageFunctions : IFunctionCalled, INameToIdResolver, IAIFunctionsSource
    {
        ...
        {
            _nodeDict = nodes
                .Where(o => o.Type.Contains("Microsoft.CognitiveServices/accounts/TextAnalytics", StringComparison.InvariantCultureIgnoreCase))
                .ToDictionary(n => n.Name, n => n.Id);
        }

        [KernelFunction]
        public int ResolveLanguageAccountNameToID(string name)
        {
            FunctionCalled?.Invoke(this, new FunctionCallEventArgs($"{nameof(ResolveLanguageAccountNameToID)}(\"{name}\") "));
